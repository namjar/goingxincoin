...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans) {
  ...
  while (mapOrphanTransactions.size() > nMaxOrphans) {
    ...
    map < uint256,
    CDataStream *  > ::iterator it = mapOrphanTransactions.lower_bound(randomhash);
...
int CMerkleTx::SetMerkleBranch(const CBlock * pblock) {
  ...
  map < uint256,
  CBlockIndex *  > ::iterator mi = mapBlockIndex.find(hashBlock);
...
int CTxIndex::GetDepthInMainChain()const{
  ...
  map < uint256,
  CBlockIndex *  > ::iterator mi = mapBlockIndex.find(block.GetHash());
  if (mi == mapBlockIndex.end())
    return 0;
  CBlockIndex * pindex = ( * mi).second;
  if (!pindex || !pindex->IsInMainChain())
...
int64 GetProofOfWorkReward(unsigned int nBits, unsigned int nHeight) {
  ...
  if (nHeight < BLOCK_REWARD_2) {
    ...
    bnUpperBound = 3;
  } else if (nHeight < BLOCK_REWARD_4) {
    bnUpperBound = 4;
  }
...
int64 GetProofOfStakeReward(int64 nCoinAge) {
  static int64 nRewardCoinYear = 0.1 * CENT; // creation amount per coin-year
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos) {
  ...
  map < uint256,
  CBlockIndex *  > ::iterator miPrev = mapBlockIndex.find(hashPrevBlock);
  ...
  map < uint256,
  CBlockIndex *  > ::iterator mi = mapBlockIndex.insert(make_pair(hash, pindexNew)).first;
  if (pindexNew->IsProofOfStake())
...
bool CBlock::AcceptBlock() {
  ...
  map < uint256,
  CBlockIndex *  > ::iterator mi = mapBlockIndex.find(hashPrevBlock);
  if (mi == mapBlockIndex.end())
...
=======
    txNew.vout[0] = 1;
>>>>>>> dc59dc3a60829d95662575e3723528b958ecdbd6
    CBlock block;
  ...
  if (strCommand == "version") {
    ...
    BOOST_FOREACH(const CInv & inv, vInv) {
      ...
      if (inv.type == MSG_BLOCK) {
        ...
        map < uint256,
        CBlockIndex *  > ::iterator mi = mapBlockIndex.find(inv.hash);
        if (mi != mapBlockIndex.end()) {
          CBlock block;
        ...
        // Send stream from relay memory
        {
          LOCK(cs_mapRelay);
          map < CInv,
          CDataStream > ::iterator mi = mapRelay.find(inv);
          if (mi != mapRelay.end())
    ...
    if (locator.IsNull()) {
      ...
      map < uint256,
      CBlockIndex *  > ::iterator mi = mapBlockIndex.find(hashStop);
      if (mi == mapBlockIndex.end())
        return true;
      pindex = ( * mi).second;
    } else {
    ...
    CRequestTracker tracker;{
      ...
      map < uint256,
      CRequestTracker > ::iterator mi = pfrom->mapRequests.find(hashReply);
...
CBlock * CreateNewBlock(CWallet * pwallet, bool fProofOfStake) {
  ...
  if (fProofOfStake) // attemp to find a coinstake
  {
    ...
    if (nSearchTime > nLastCoinStakeSearchTime) {
      if (pwallet->CreateCoinStake( * pwallet, pblock->nBits, nSearchTime - nLastCoinStakeSearchTime, txCoinStake)) {
        ...
        if (txCoinStake.nTime >= max(pindexPrev->GetMedianTimePast() + 1, pindexPrev->GetBlockTime() - nMaxClockDrift)) { // make sure coinstake would meet timestamp protocol
  ...
  int64 nFees = 0;{
    ...
    map < uint256,
    vector < COrphan *  >  > mapDependers;
    multimap < double,
    CTransaction *  > mapPriority;
    ...
    while (!mapPriority.empty()) {
      ...
      map < uint256,
      CTxIndex > mapTestPoolTmp(mapTestPool);
      MapPrevTx mapInputs;
      bool fInvalid;
  ...
  while (fGenerateBitcoins || fProofOfStake) {
    ...
    if (fProofOfStake) {
      ...
      if (pblock->IsProofOfStake()) {
        ...
        Sleep(60 * 1000 + GetRand(4 * 60 * 1000));
      }
      Sleep(pos_timio);
      continue;
    }
